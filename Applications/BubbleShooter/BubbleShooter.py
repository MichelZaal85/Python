...
def keydown(key):
    ...
    inputs = {"right": -FIRING_ANGLE_VEL_INC,
              "left": FIRING_ANGLE_VEL_INC,
              "space": a_bubble.fire_bubble(FIRING_ANGLE_VEL_INC)}
    for k in inputs:
        if key == simplegui.KEY_MAP[k]:
            # Windows simpleguitk bug
            # if firing_angle_vel <= -.02 or firing_angle_vel <= .02:
                firing_angle_vel += inputs[k]
...
def draw(canvas):
    ...
    # draw a_bubble and stuck bubbles
