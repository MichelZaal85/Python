...
def draw(canvas):
    ...
    # update ball
    # if ball_pos[1] >= WIDTH - BALL_RADIUS: #and ball_pos[1] <= 600:
    #     if ball_pos[1] >= paddle2_pos -80 and ball_pos[1] <= paddle2_pos:
    #         print('Paddle2 collide')
    ...
    # Gutters
    if ball_pos[0] <= 0: # hits left gutter
        score2 += 1
        spawn_ball(RIGHT)
        ...
        if ball_pos[1] >= paddle2_pos -80 and ball_pos[1] <= paddle2_pos:
        	...
        	spawn_ball(LEFT)
        score1 += 1
        print(ball_pos[0],ball_pos[1]) # 300,200
    ...
    if paddle2_pos-80 <= 0 or paddle2_pos >= 400:
        paddle2_vel = 0

    # draw paddles
    canvas.draw_polyline([(HALF_PAD_WIDTH,paddle1_pos),(HALF_PAD_WIDTH, paddle1_pos-80)],8,'red')
    canvas.draw_polyline([(WIDTH-HALF_PAD_WIDTH,paddle2_pos),(WIDTH-HALF_PAD_WIDTH, paddle2_pos-80)],8,'lime')

    # draw scores
    canvas.draw_text(str(score1) + '/' + str(score2),(100,50), 25,'green')
...
def keydown(key):
    ...
    if key == simplegui.KEY_MAP['w'] and paddle2_pos >= 80: # UP
        paddle2_vel -= 3
    if key == simplegui.KEY_MAP["s"] and paddle2_pos <= 400: # DWN
        paddle2_vel += 3
...
as described in the "Pong" video.) 

When the ball touches a gutter, use either spawn_ball(LEFT) or spawn_ball(RIGHT) to respawn the ball in the center 
of the table headed towards the opposite gutter.
Next, add code that draws the left and right paddles in their respective gutters. 
...
should reference two global variables that contain the vertical velocities of the paddles. 
...
move part of the paddle off of the screen. 
...
if ball_pos[1] < (paddle2_pos+80) and ball_pos[1] > (80) and ball_pos[0] = WIDTH:
ball_vel[0] = ball_vel[0]*(-1)
This gets bounced off alright, but only at one position.
I've been scouting around here looking for a solution to collision, would appreciate further assistance on this.



#check if the ball hit the gutter
if the ball hit the gutter:
    #check if it hit the paddle
    if the ball hit the paddle:
        bounce
    else:
        respawn and score
elif the ball hit the other gutter:
    etc
...
if the ball hit the gutter:
    ...
    #by comparing
    #   the ball's y,
    #   and the paddle's top,
    #   and the paddle's bottom
    if the ball hit the paddle:
        bounce
    else:
        respawn and score
