...
def draw(canvas):
    ...
    elif ball_pos[0] >= WIDTH - BALL_RADIUS+4:
        ball_vel[0] = -ball_vel[0]
        print('right BANG!')
        # input('...Game!...')
    ...
    # draw paddles
    # Left Paddle
    canvas.draw_polyline([(HALF_PAD_WIDTH,paddle1_vel),(HALF_PAD_WIDTH, paddle1_vel + 80)],8,'red')    
    
    # Right Paddle
    # canvas.draw_polyline([(WIDTH-HALF_PAD_WIDTH,0),(WIDTH-HALF_PAD_WIDTH, PAD_HEIGHT)],8,'red')


    # determine whether paddle and ball collide    
    
    # draw scores
    # canvas.draw_text(score1, score2)
...
def keydown(key):
    ...
    if key == simplegui.KEY_MAP['down']:
        paddle1_vel += 10
        # print('vel:',paddle1_vel)
        print(paddle1_vel)
        if paddle1_vel-80 >= 240:
            print('Bottom!, stop!!')
...
a fixed velocity (for now). Ignore the parameter direction at this point.
...
Add randomization to the velocity in spawn_ball(direction) The velocity of the ball should be upwards and 
towards the right if direction == RIGHT and upwards and towards the left if direction == LEFT. The exact 
values for the horizontal and vertical components of this velocity should be generated using random.randrange(). 
