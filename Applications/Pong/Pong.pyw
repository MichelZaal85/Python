...
def spawn_ball(direction):
    ...
    ball_vel_x = random.randrange(120) / 60
    # ball_vel_y = random.randrange(60, 180) / 60
    ball_vel_y = random.randrange(60) / 60
    if direction:
...
def draw(canvas):
    ...
    # draw paddles
    canvas.draw_polyline([(HALF_PAD_WIDTH, paddle1_pos),
                          (HALF_PAD_WIDTH, paddle1_pos - PAD_HEIGHT)],
                         PAD_WIDTH, 'red')
    canvas.draw_polyline([(WIDTH - HALF_PAD_WIDTH, paddle2_pos),
